    ...
    <div class="col-lg-10 offset-lg-1">
        <div class="card">
            <div class="card-header bg-warning text-white">
                <h5 class="mb-0"><i class="fas fa-edit me-2"></i> Edit Penilaian</h5>
            </div>
            <div class="card-body">
                <form action="{{ route('penilaian.update', $penilaian->id_penilaian) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('id_tim') is-invalid @enderror" 
                                    ...
                                    @foreach($tim as $t)
                                        <option value="{{ $t->id_tim }}" {{ old('id_tim', $penilaian->id_tim) == $t->id_tim ? 'selected' : '' }}>
                                            {{ $t->nama_tim }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('id_juri') is-invalid @enderror" 
                                    ...
                                    @foreach($juri as $j)
                                        <option value="{{ $j->id_juri }}" {{ old('id_juri', $penilaian->id_juri) == $j->id_juri ? 'selected' : '' }}>
                                            {{ $j->user->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('id_kriteria') is-invalid @enderror" 
                                    ...
                                    @foreach($kriteria as $k)
                                        <option value="{{ $k->id_kriteria }}" {{ old('id_kriteria', $penilaian->id_kriteria) == $k->id_kriteria ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control @error('nilai') is-invalid @enderror" 
                                       id="nilai" name="nilai" value="{{ old('nilai', $penilaian->nilai) }}" min="0">
                                @error('nilai')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('komentar') is-invalid @enderror" 
                                  id="komentar" name="komentar" rows="3">{{ old('komentar', $penilaian->komentar) }}</textarea>
                        @error('komentar')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                            ...
                            <p class="mb-2">
                                <a href="{{ asset('uploads/penilaian/' . $penilaian->dokumen_pendukung) }}" 
                                   ...
                                   class="btn btn-sm btn-outline-info">
                                    <i class="fas fa-file me-1"></i> Lihat Dokumen Saat Ini
                                </a>
                            </p>
                        @endif
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" 
                            ...
                            <option value="selesai" {{ old('status', $penilaian->status) == 'selesai' ? 'selected' : '' }}>Selesai</option>
                        </select>
                        @error('status')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
